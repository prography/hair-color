...
class Dataset:
    def __init__(self, batch_size, folder):
        ...
        train_names, val_names, test_names = self.train_valid_test_split(
            os.listdir(os.path.join(folder, 'images')), ratio=(0.95, 0.02, 0.03))
    ...
    def file_paths_to_images(self, folder, file_names):
        ...
        for name in file_names:
            ...
            _input = np.array(cv2.imread(input_file)[:, :, ::-1])
    ...
    def next_batch(self):
        ...
        return np.array(inputs, dtype=np.float32), np.array(targets, dtype=np.uint8)
    ...
    def val_set(self):
        ...
        return np.array(self.val_inputs, dtype=np.float32), np.array(self.val_targets, dtype=np.uint8)

    def test_set(self):
        return np.array(self.test_inputs, dtype=np.float32), np.array(self.test_targets, dtype=np.uint8)